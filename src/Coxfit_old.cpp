...
extern "C" {
  ...
  double linNegLogLikelihoodCalc(int *nrow, double *eta, double *y, double *wts)
  {
    ...
    for(int i = 0; i < nrow[0]; i++)
    {
      squareSum = squareSum + wts[i] * pow(eta[i] - y[i], 2)/2.0;
    }
  ...
  void linSolver(double *X, double *y, int* index, int *nrow, int *ncol,
                 ...
                 double *step, int *reset, double *groupW, double *wts)
  {
    double *theta = new double[ncol[0]];
    int startInd = 0;
    double zeroCheck = 0;
    ...
    for(int i = 0; i < numGroup[0]; i++)
    {
      if(useGroup[i] == 1)
      {
        ...
        // Calculating Null Gradient
        linGradCalc(nrow, etaNull, y, wts, ldot);
        ...
        else
        {
          ...
          while(count <= innerIter[0] && check > thresh[0])
          {
            ...
            linGradCalc(nrow, eta, y, wts, ldot);
            ...
            diff = -1;
            //	      t = 0.5;
            Lold = linNegLogLikelihoodCalc(nrow, eta, y, wts);
            ...
            while(diff < 0)
            {
              ...
              Lnew = linNegLogLikelihoodCalc(nrow, etaNew, y, wts);
  ...
  int linNest(double *X, double* y, int* index, int *nrow, int *ncol,
              ...
              double *lambda2, double *beta, int *innerIter, int *outerIter,
              double *thresh, double *outerThresh, double *eta, double *gamma,
              int *betaIsZero, double *step, int *reset, double *groupW, double *weights)
  {
    double* prob = NULL;
    ...
    while(groupChange == 1)
    {
      ...
      linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen,
                lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta,
                betaIsZero, groupChange, isActive, useGroup, step, reset, groupW,
                weights);
      ...
      while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      {
        ...
        linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd,
                  ...
                  nullBeta, gamma, eta, betaIsZero, groupChange,
                  isActive, tempIsActive, step, reset, groupW, weights);
  ...
  void pCalc(int *nrow, double *eta, double *prob, double *wts)
  {
    for(int i = 0; i < nrow[0]; i++)
    {
  ...
  double logitNegLogLikelihoodCalc(int *nrow, double *prob, int *y, double *wts)
  {
    ...
    for(int i = 0; i < nrow[0]; i++)
    {
      logLik = logLik + wts[i] * y[i] * log(prob[i]) + (1 - y[i]) * log(1 - prob[i]);
    }
  ...
  void logitSolver(double *X, int *y, int* index, int *nrow, int *ncol, int *numGroup,
                   double *beta, int *rangeGroupInd, int *groupLen, double *lambda1, double *lambda2,
                   int *innerIter, double *thresh, double *ldot, double *nullBeta, double *gamma,
                   double *eta, int* betaIsZero, int& groupChange, int* isActive, int* useGroup,
                   double *prob, double *betaZero, double *step, double *groupW, double *wts)
  {
    double *theta = new double[ncol[0]];
    double *thetaNew = new double[ncol[0]];
    int startInd = 0;
    ...
    for(int i = 0; i < numGroup[0]; i++)
    {
      if(useGroup[i] == 1)
      {
        ...
        // Calculating Null Gradient
        pCalc(nrow, etaNull, prob, wts);
        logitGradCalc(nrow, prob, y, wts, ldot);
        ...
        else
        {
          ...
          while(count <= innerIter[0] && check > thresh[0])
          {
            ...
            //	      t = 0.5;
            pCalc(nrow, eta, prob, wts);
            Lold = logitNegLogLikelihoodCalc(nrow, prob, y, wts);
            ...
            while(diff < 0)
            {
              ...
              pCalc(nrow, etaNew, prob, wts);
              Lnew = logitNegLogLikelihoodCalc(nrow, prob, y, wts);
    ...
    betaZeroSolve(nrow, betaZero, eta, prob, thresh, innerIter, y, wts);
  ...
  int logitNest(double *X, int* y, int* index, int *nrow, int *ncol,
                ...
                int *betaIsZero, double* betaZero, double *step, double *groupW,
                double *weights)
  {
    ...
    while(groupChange == 1)
    {
      ...
      logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd,
                  ...
                  prob, betaZero, step, groupW, weights);
      ...
      while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      {
        ...
        logitSolver(X, y, index, nrow, ncol, numGroup, beta,
                    rangeGroupInd, groupLen, lambda1, lambda2,
                    innerIter, thresh, ldot, nullBeta, gamma,
                    eta, betaIsZero, groupChange, tempIsActive,
                    isActive, prob, betaZero, step, groupW, weights);
        outermostCheck = 0;
        for(int i = 0; i < p; i++)
        {
          outermostCheck = outermostCheck + fabs(outerOldBeta[i] - beta[i]);
