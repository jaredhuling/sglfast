...
extern "C" {
  ...
  double negLogLikelihoodCalc(int *riskSetInd, int *riskSet, int *numDeath, int *nDeath, 
                              int *nrow, int *ncol, double *eta, double* V, int *deathInd, int *totDeath)
  {
    double first = 0;
    double second = 0;
  ...
  void Solver(double *X, int* index, int *nrow, int *ncol, int *numGroup, double *beta, 
              int *rangeGroupInd, int *groupLen, double *lambda1, double *lambda2, 
              int *innerIter, double *thresh, double *ldot, double *nullBeta, 
  ...
  void linGradCalc(int *nrow, double *eta, double *y, double *wts, double *ldot)
  {
    for(int i = 0; i < nrow[0]; i++)
    {
      ldot[i] = wts[i] * (eta[i] - y[i])/nrow[0];  /* OR MAYBE NOT? */
    }
  }
  
  
  double linNegLogLikelihoodCalc(int *nrow, double *eta, double *y, double *wts)
  {
    double squareSum = 0;
    
    for(int i = 0; i < nrow[0]; i++)
    {
      squareSum = squareSum + wts[i] * pow(eta[i] - y[i], 2)/2; 
    }
  ...
  void linSolver(double *X, double *y, int* index, int *nrow, int *ncol, 
                 int *numGroup, double *beta, int *rangeGroupInd, int *groupLen, 
                 double *lambda1, double *lambda2, int *innerIter, double *thresh, 
                 double *ldot, double *nullBeta, double *gamma, double *eta, 
                 int* betaIsZero, int& groupChange, int* isActive, int* useGroup, 
                 double *step, int *reset, double *weights)
  {
    double *theta = new double[ncol[0]];
    int startInd = 0;
    double zeroCheck = 0;
    ...
    for(int i = 0; i < numGroup[0]; i++)
    {
      if(useGroup[i] == 1)
      {
        ...
        // Calculating Null Gradient
        linGradCalc(nrow, etaNull, y, weights, ldot);
        ...
        else
        {
          ...
          while(count <= innerIter[0] && check > thresh[0])
          {
            ...
            linGradCalc(nrow, eta, y, weights, ldot);
            ...
            diff = -1;
            //	      t = 0.5;
            Lold = linNegLogLikelihoodCalc(nrow, eta, y, weights);
            ...
            while(diff < 0)
            {
              ...
              Lnew = linNegLogLikelihoodCalc(nrow, etaNew, y, weights);
  ...
  int linNest(double *X, double* y, int* index, int *nrow, int *ncol, int *numGroup, 
              int *rangeGroupInd, int *groupLen, double *lambda1, double *lambda2, 
              double *beta, int *innerIter, int *outerIter, double *thresh, double *outerThresh, 
              double *eta, double *gamma, int *betaIsZero, double *step, int *reset, double *weights)
  {
    double* prob = NULL;
    ...
    while(groupChange == 1)
    {
      ...
      linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, 
                groupLen, lambda1, lambda2, innerIter, thresh, ldot, 
                nullBeta, gamma, eta, betaIsZero, groupChange, isActive, 
                useGroup, step, reset, weights);
      ...
      while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      {
        ...
        linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, 
                  groupLen, lambda1, lambda2, innerIter, thresh, ldot, 
                  nullBeta, gamma, eta, betaIsZero, groupChange, isActive, 
                  tempIsActive, step, reset, weights);
  ...
  void logitGradCalc(int *nrow, double *prob, int *y, double *wts, double *ldot)
  {
    for(int i = 0; i < nrow[0]; i++)
    {
      ldot[i] = wts[i] * (-y[i] + prob[i])/nrow[0]; /* OR MAYBE NOT? */
    }
  }
  ...
  double logitNegLogLikelihoodCalc(int *nrow, double *prob, int *y, double *wts)
  {
    double logLik = 0;
    
    for(int i = 0; i < nrow[0]; i++)
    {
      logLik = logLik + wts[i] * y[i] * log(prob[i]) + wts[i] * (1 - y[i]) * log(1 - prob[i]);
    }
    
    return -logLik/nrow[0];  /* OR MAYBE NOT? */
  }
  
  void betaZeroSolve(int *nrow, double *betaZero, double *eta, double *prob, double *thresh, int *innerIter, int *y, double *wts)
  {
    double diff = 10;
    double num = 0;
    double denom = 0;
    ...
    while(pow(diff,2) > pow(thresh[0],2) && count < innerIter[0])
    {
      ...
      for(int i = 0; i < nrow[0]; i++)
      {
        num = num + wts[i] * (y[i] - prob[i]);
        denom = denom + wts[i] * prob[i] * (1 - prob[i]);
      }
      diff = num/denom;
  ...
  void logitSolver(double *X, int *y, int* index, int *nrow, int *ncol, int *numGroup, 
                   double *beta, int *rangeGroupInd, int *groupLen, double *lambda1, 
                   double *lambda2, int *innerIter, double *thresh, double *ldot, 
                   double *nullBeta, double *gamma, double *eta, int* betaIsZero, 
                   int& groupChange, int* isActive, int* useGroup, double *prob, 
                   double *betaZero, double *step, double *weights)
  {
    double *theta = new double[ncol[0]];
    double *thetaNew = new double[ncol[0]];
    int startInd = 0;
    ...
    for(int i = 0; i < numGroup[0]; i++)
    {
      if(useGroup[i] == 1)
      {
        ...
        logitGradCalc(nrow, prob, y, weights, ldot);
        ...
        else
        {
          ...
          while(count <= innerIter[0] && check > thresh[0])
          {
            ...
            //	      t = 0.5;
            pCalc(nrow, eta, prob);
            Lold = logitNegLogLikelihoodCalc(nrow, prob, y, weights);
            ...
            while(diff < 0)
            {
              ...
              Lnew = logitNegLogLikelihoodCalc(nrow, prob, y, weights);
    ...
    betaZeroSolve(nrow, betaZero, eta, prob, thresh, innerIter, y, weights);
  ...
  int logitNest(double *X, int* y, int* index, int *nrow, int *ncol, int *numGroup, 
                int *rangeGroupInd, int *groupLen, double *lambda1, double *lambda2, 
                double *beta, int *innerIter, int *outerIter, double *thresh, double *outerThresh, 
                double *eta, double *gamma, int *betaIsZero, double* betaZero, double *step, double *weights)
  {
    ...
    while(groupChange == 1)
    {
      ...
      logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, 
                  groupLen, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, 
                  gamma, eta, betaIsZero, groupChange, isActive, useGroup, 
                  prob, betaZero, step, weights);
      ...
      while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      {
        ...
        logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, 
                    groupLen, lambda1, lambda2, innerIter, thresh, ldot, 
                    nullBeta, gamma, eta, betaIsZero, groupChange, tempIsActive, 
                    isActive, prob, betaZero, step, weights);
