run_sgl_logit <- function(data, group.length,  thresh = 0.0001, lambda1 = 0.2, lambda2 = 0.01,
                          ...
                          groupW = NULL, weights = rep(1, NROW(data)))
{
  X <- data$x
  y <- data$y
  n <- nrow(X)
  ...
  junk <- .C("logitNest", X = as.double(as.vector(X)), y = as.integer(y), 
             ...
             lambda1 = as.double(lambda1), lambda2 = as.double(lambda2), beta = as.double(beta.old),
             ...
             betaZero = as.double(intercept), step = as.double(step), groupW = as.double(groupW),
